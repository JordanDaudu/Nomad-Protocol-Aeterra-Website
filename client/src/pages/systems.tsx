...
type NavigateOptions = {
    /**
     * When true (default), the mobile tree drawer closes after navigation.
     * Use false for folder interactions so the drawer stays open.
     */
...
function TreeNode({
    ...
    node: DocNode;
    currentRoute: string;
    onNavigate: (route: string, options?: NavigateOptions) => void;
    isMobile: boolean;
    depth?: number;
}) {
    ...
    if (node.type === "doc") {
        return (
            <button
                onClick={() => onNavigate(node.route)} // docs close the drawer by default
        ...
        <div>
            <button
                onClick={() => {
                    ...
                    // Mobile: folder tap only expands/collapses (NO navigation, drawer stays open)
                    if (isMobile) return;

                    // Desktop: navigate to folder route, but never close drawer for folders
                    if (node.route !== "/systems") {
                        onNavigate(node.route, { closeSidebar: false });
                    }
                }}
                className={`tree-item w-full text-left flex items-center gap-2 px-3 py-1.5 text-sm font-terminal transition-all duration-200 ${
                    isActive
    ...
    return (
        <div>
            <div className="border-b border-border pb-4 mb-6">
                ...
                {doc.summary && <p className="text-muted-foreground font-terminal text-sm">{doc.summary}</p>}
...
function FolderListing({
    ...
    return (
        <div>
            <h1 className="text-3xl font-bold font-display tracking-tight text-primary mb-2">{node.title}</h1>
            {node.summary && <p className="text-muted-foreground font-terminal text-sm mb-6">{node.summary}</p>}
                ...
                <div className="mb-6">
                    ...
                    <div className="grid gap-2">
                        {folders.map((f) => (
                            <button
                                key={f.route}
                                onClick={() => onNavigate(f.route, { closeSidebar: false })} // folder -> keep drawer open
                    ...
                    {f.title}
                                ...
                                {f.summary && <p className="text-muted-foreground text-sm mt-1 ml-6">{f.summary}</p>}
                            </button>
                        ))}
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div className="grid gap-2">
                        {docs.map((d) => (
                            <button
                                key={d.route}
                                onClick={() => onNavigate(d.route)} // doc -> close drawer (default)
                                className="text-left p-3 border border-border hover:border-primary/50 hover:bg-primary/5 transition-all group"
                    ...
                    {d.title}
                                ...
                                {d.summary && <p className="text-muted-foreground text-sm mt-1 ml-6">{d.summary}</p>}
                            </button>
                        ))}
                    </div>
                </div>
...
export default function SystemsArchivePage() {
    ...
    // Mobile detector without deprecated APIs or ts-expect-error directives
    ...
    const handleNavigate = useCallback(
        (route: string, options?: NavigateOptions) => {
            ...
            // Close only when we *want* to (docs). Folders keep the drawer open.
    ...
    if (isLoading) {
        return (
            <div className="flex items-center justify-center h-64">
                ...
                <span className="ml-3 font-terminal text-muted-foreground text-sm">
          LOADING_SYSTEMS_ARCHIVE...
        </span>
            </div>
        );
    }
    ...
    if (error || !tree) {
        return (
            <div className="flex items-center justify-center h-64">
        <span className="font-terminal text-destructive text-sm">
          ERROR: SYSTEMS_ARCHIVE_UNAVAILABLE
        </span>
            </div>
        );
    }
    ...
    const renderContent = () => {
                ...
                <div>
                    ...
                    <div className="grid gap-2">
                        {searchResults.map((doc) => (
                            <button
                                key={doc.route}
                                onClick={() => handleNavigate(doc.route)} // doc -> close drawer
                                className="text-left p-3 border border-border hover:border-primary/50 hover:bg-primary/5 transition-all group"
                            >
                <span className="font-display font-bold group-hover:text-primary transition-colors">
                  {doc.title}
                </span>
                                {doc.summary && <p className="text-muted-foreground text-sm mt-1">{doc.summary}</p>}
                            </button>
                        ))}
                    </div>
                </div>
        ...
        if (showSearch && searchQuery.trim()) {
            return (
                <div className="text-center py-12">
                    <p className="font-terminal text-muted-foreground">NO_RESULTS: "{searchQuery}"</p>
                </div>
            );
        }
        ...
        return (
            <div className="text-center py-12">
                <p className="font-terminal text-muted-foreground">DOC_NOT_FOUND: {currentRoute}</p>
            </div>
        );
    };
            ...
            <span className="font-terminal text-[10px] text-muted-foreground uppercase tracking-widest">
              ...
              <span className="font-terminal text-[10px] text-muted-foreground uppercase tracking-widest">
                Archive Tree
                        ...
                        <button
                            onClick={() => handleNavigate("/systems", { closeSidebar: false })} // keep drawer open
                            className={`tree-item w-full text-left flex items-center gap-2 px-3 py-1.5 text-sm font-terminal transition-all duration-200 ${
